...
const char *pin_mapping = R"({"1":5,"2":4,"3":0,"4":2,"5":14,"6":12,"7":13,"8":15})";

ESP8266WebServer server(80);

void handleRoot();
void handleNotFound();
String save_state(const StaticJsonDocument<150>&);
StaticJsonDocument<150> load_state();
String gen_res_message(const String&);
void routes();

String save_state(const StaticJsonDocument<150>& doc) {
...
String gen_res_message(const String& message) {
...
void setup(void){
  ...
  auto documentRoot = state.as<JsonObject>();
  ...
  size_t header_key_size = sizeof(header_keys) / sizeof(char *);
  // ask server to track these headers
  server.collectHeaders(header_keys, header_key_size);
